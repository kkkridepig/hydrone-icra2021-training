...
class Env():
    def __init__(self, action_dim=3):
        ...
        self.position = Pose()
        self.pub_cmd_vel = rospy.Publisher('/hydrone_aerial_underwater0/cmd_vel', Twist, queue_size=5)
        self.sub_odom = rospy.Subscriber('/hydrone_aerial_underwater0/ground_truth/odometry', Odometry, self.getOdometry)
        self.reset_proxy = rospy.ServiceProxy('gazebo/reset_world', Empty)
        self.pub_pose = rospy.Publisher("/hydrone_aerial_underwater0/ground_truth/pose", Pose, queue_size=5)
        self.pub_end = rospy.Publisher("/hydrone_aerial_underwater0/end_testing", Bool, queue_size=5)
        self.pub_reward = rospy.Publisher("/hydrone_aerial_underwater0/rewarded", Bool, queue_size=5)
    ...
    def getState(self, scan, past_action):        
        ...
        if min_range > min(scan_range) or self.position.z < -0.9 or self.position.z > 4.8:
            # print(scan_range)
            done = True
    ...
    def step(self, action, past_action):        
        ...
        while data is None:
            try:
                data = rospy.wait_for_message('/hydrone_aerial_underwater0/scan', LaserScan, timeout=5)
            except:
                pass
